...
class ArcBallUtil(ArcBall):
    ...
    def onScroll(self, scroll_delta, zoom_sensitivity=0.001):
        """
        Mouse scroll event to zoom in and out.
        scroll_delta: positive for zoom in, negative for zoom out
        zoom_sensitivity: controls how fast zooming happens
        """
        # Clamp zoom factor to avoid flipping or negative scale
        scale_factor = 1.0 + zoom_sensitivity * scroll_delta
        scale_factor = max(0.1, min(scale_factor, 10.0))

        # Scale the translation vector (zoom along view direction)
        # self.Transform[:3, 3] *= scale_factor

        # If you want to scale the whole object (including rotation), you could do:
        self.Transform[:3, :3] *= scale_factor
        # But usually, zooming only affects translation.

        # Example: call a function if zooming in
        # if scroll_delta > 0:
        #     self.onZoomIn()
